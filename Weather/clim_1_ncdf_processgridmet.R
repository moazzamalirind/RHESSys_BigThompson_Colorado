#Source : https://github.com/Fire-and-Dryland-Ecosystems-Lab/EcohydrologyScripts/blob/main/Rscripts/CARB_Workflow/clim_1_ncdf_processgridmet.R#L1
...
#set working directory (change as per your location)
setwd ("F:/Corrected_RHESSys/Climate")

# Import basin raster. ()
basin = rast("Data/Basin")          #try use same raster you using for RHESSys.
...
# OR
clim_files = list.files(path = "GridMET_Raw",pattern = "agg_met_", full.names = T)

# destination folder for cropped + edited ncdf clim files. You need to make an empty folder (e.g. clim) and then it point to it.
#Without folder presence it will through error.
clim_dest = "clim"
...
basin_extent = ext(basin_vect_unproj) + 0.04
#adding buffer of 0.04 around the extent to cover everything in watershed
